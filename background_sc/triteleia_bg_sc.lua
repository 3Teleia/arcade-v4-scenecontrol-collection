...
function create_bg (filename)
	...
	if filename ~= nil and filename ~= '' and type(filename) == "string" then
		...
		-- Sets the image's width and height respectively
		...
		bg_layer_channels[filename] = Channel.named(filename).keyframe().setDefaultEasing('outconst').addKey(0,bg_top_layer)
		...
		-- Makes the new canvas use the alpha values given by the previously defined channel
		new_canvas.alpha = bg_alpha_channels[filename]
		
		-- Saves the canvas for later usage (for bgshow and bgsetlayer)
		bg_canvases[filename] = new_canvas
		
		-- Parents the new BG to the canvas so that changing the alpha or layering of the canvas would affect the BG as well
...
function bg_set_layer(timing, filename, layer, layer_group)
	if filename ~= nil and filename ~= '' and type(filename) == "string" then
		...
		local start_layer = bg_canvases[filename].sort.valueAt(timing)
		bg_layer_channels[filename]
		.addKey(timing-1, start_layer)
		.addKey(timing, layer)
		
		if type(layer_group) == "string" and (string.lower(layer_group) == "foreground" or string.lower(layer_group) == "background") then
			local start_group = bg_canvases[filename].layer.valueAt(timing)
			bg_layer_group_channels[filename].addKey(timing-1, start_group)
