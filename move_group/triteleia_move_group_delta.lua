...
function move_group(timing, end_timing, timing_group, x, y, z, easing)
	if x ~= 0 then
		if x_channels[timing_group] ~= nil then
			x_channels[timing_group].addKey(timing, x_channels[timing_group].valueAt(timing-1)).addKey(end_timing, x_channels[timing_group].valueAt(timing-1)+x, easing)
		else 
	...
	if y ~= 0 then
		if y_channels[timing_group] ~= nil then
			y_channels[timing_group].addKey(timing, y_channels[timing_group].valueAt(timing-1)).addKey(end_timing, y_channels[timing_group].valueAt(timing-1)+y, easing)
		else 
	...
	if z ~= 0 then
		if z_channels[timing_group] ~= nil then
			z_channels[timing_group].addKey(timing, z_channels[timing_group].valueAt(timing-1)).addKey(end_timing, z_channels[timing_group].valueAt(timing-1)+z, easing)
		else 
